...
import { ExternalLink, Inbox, Tag } from "lucide-react";
...
function Badge({ value, color }: { value: string; color: string }) {
	return (
		<span className={cn("px-1.5 py-0.5 rounded text-[9px] font-medium", color)}>
			{value}
		</span>
	);
}
		...
		return (
			<div className={cn("bg-card border border-border rounded-sm", className)}>
				...
				<div className="p-6 text-center">
					...
					<p className="text-[11px] text-muted-foreground">No UTM campaign data in this period</p>
	...
	return (
		<div className={cn("bg-card border border-border rounded-sm", className)}>
			<div className="px-3 py-2 border-b border-border flex items-center justify-between">
				...
				<span className="text-[10px] text-muted-foreground">{data.length} campaigns</span>
			</div>
				...
				<table className="w-full text-[11px]">
					<thead>
						<tr className="border-b border-border bg-muted/20">
							<th className="px-3 py-1.5 text-left text-[10px] font-medium text-muted-foreground">Source / Medium</th>
							<th className="px-3 py-1.5 text-left text-[10px] font-medium text-muted-foreground">Campaign</th>
							<th className="px-3 py-1.5 text-right text-[10px] font-medium text-muted-foreground">Visits</th>
							<th className="px-3 py-1.5 text-right text-[10px] font-medium text-muted-foreground">Share</th>
						</tr>
					</thead>
					<tbody className="divide-y divide-border/40">
						{data.map((row, i) => {
							const mediumColor = mediumColors[row.medium?.toLowerCase() || "direct"] || "bg-muted text-muted-foreground";
							return (
								<tr key={i} className="relative group hover:bg-muted/30 transition-colors">
									<td className="px-3 py-2">
										<div className="flex items-center gap-1.5 flex-wrap">
											...
											{row.medium && (
												<Badge value={row.medium} color={mediumColor} />
											)}
									...
									<td className="px-3 py-2">
										{row.campaign ? (
											<span className="text-foreground truncate max-w-[180px] block">{row.campaign}</span>
										) : (
									...
									<td className="px-3 py-2 text-right">
										<div className="flex items-center justify-end gap-2">
											...
											<span className="font-medium tabular-nums text-foreground">{row.visits.toLocaleString()}</span>
										</div>
									</td>
