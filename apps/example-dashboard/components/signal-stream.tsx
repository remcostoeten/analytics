...
import { Radio, Globe, Server, Clock, Hash, ExternalLink, ChevronDown, ChevronUp, Inbox } from "lucide-react"
            ...
            {metadata.statusCode && (
              <div className="flex items-center gap-2">
                ...
                <span className={cn(
                  ...
                  Number(metadata.statusCode) >= 300 && Number(metadata.statusCode) < 400 && "text-blue-600 dark:text-blue-400",
                  Number(metadata.statusCode) >= 400 && Number(metadata.statusCode) < 500 && "text-amber-600 dark:text-amber-400",
  ...
  const filteredSignals = useMemo(() => {
    return signals.filter(signal => {
      ...
      if (filter) {
        ...
        const matchesEndpoint = signal.metadata?.endpoint?.toString().toLowerCase().includes(searchLower)
