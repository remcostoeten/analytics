...
export function DashboardHeader({
  ...
  return (
    <header className={cn("flex items-center justify-between h-10 px-3 border-b border-border bg-card", className)}>
      ...
      <DropdownMenuContent align="end" className="w-48">
        ...
        {projects?.map((project) => (
          <DropdownMenuItem
            ...
            className={cn("text-[11px] flex justify-between", selectedProject === project.id && "bg-muted")}
          >
            <span className="truncate flex-1">{project.id}</span>
            <span className="text-muted-foreground text-[10px] ml-2">{project.eventCount.toLocaleString()}</span>
          </DropdownMenuItem>
        ))}
