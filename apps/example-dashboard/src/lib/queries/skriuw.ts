...
export async function getSkriuwEventCounts(
	projectId: string,
	from?: Date,
	to?: Date,
	...
	const results = await sql`SELECT meta->>'eventName' as event_name, COUNT(*) as count 
     FROM events 
     WHERE ${publicTraffic(excludeVisitorId)} 
       AND type = 'event' 
       AND ts >= ${range.from} 
       AND ts <= ${range.to}
...
export async function getSkriuwEventTrend(
	projectId: string,
	from?: Date,
	to?: Date,
	excludeVisitorId?: string | null,
...
export async function getSkriuwNotesActivity(
	projectId: string,
	from?: Date,
	to?: Date,
	excludeVisitorId?: string | null,
...
export async function getSkriuwJournalActivity(
	projectId: string,
	from?: Date,
	to?: Date,
	excludeVisitorId?: string | null,
...
export async function getSkriuwAuthMetrics(
	projectId: string,
	from?: Date,
	to?: Date,
	excludeVisitorId?: string | null,
	...
	const range = getRange(from, to);
	const results = await sql`SELECT meta->>'method' as method, COUNT(*) as count 
     FROM events 
     WHERE ${publicTraffic(excludeVisitorId)} 
       AND type = 'event' 
       AND ts >= ${range.from} 
...
export async function getSkriuwRecentEvents(
	projectId: string,
	limit: number = 50,
	from?: Date,
	to?: Date,
...
export async function getSkriuwTopSearches(
	projectId: string,
	limit: number = 20,
	from?: Date,
	to?: Date,
	...
	const range = getRange(from, to);
	const results = await sql`SELECT meta->>'query' as query, COUNT(*) as count 
     FROM events 
     WHERE ${publicTraffic(excludeVisitorId)} 
       AND type = 'event' 
       AND ts >= ${range.from} 
