...
export function TechnologyBreakdown({ 
  ...
  const tabs: { id: TabId; label: string; icon: React.ElementType; data: TechItem[] }[] = [
    { id: 'browsers', label: 'Browsers', icon: Globe, data: browsers.map(b => ({ name: b.name || (b as unknown as { browser: string }).browser, version: b.version, count: b.count, percentage: b.percentage })) },
    { id: 'os', label: 'OS', icon: Monitor, data: operatingSystems.map(o => ({ name: o.name || (o as unknown as { os: string }).os, version: o.version, count: o.count, percentage: o.percentage })) },
    { id: 'languages', label: 'Languages', icon: Languages, data: languages.map(l => ({ name: l.name || (l as unknown as { language: string }).language, count: l.count, percentage: l.percentage })) },
    { id: 'screens', label: 'Screens', icon: Maximize2, data: screenSizes.map(s => ({ name: s.name || (s as unknown as { screenSize: string }).screenSize, count: s.count, percentage: s.percentage })) },
    { id: 'network', label: 'Network', icon: Wifi, data: connectionTypes.map(c => ({ name: c.name || (c as unknown as { connectionType: string }).connectionType, count: c.count, percentage: c.percentage })) },
  ]
        ...
        {activeData.length === 0 ? (
          ...
          <div className="space-y-2">
            {activeData.slice(0, 8).map((item, i) => {
              const colorClass = activeTab === 'browsers' 
