...
export function RevenueDashboard({ data }: RevenueDashboardProps) {
  ...
  const kpis = [
    { label: "Total Revenue", value: formatCurrency(data.totalRevenue), icon: DollarSign, color: "text-emerald-500" },
    { label: "Orders", value: data.transactionCount.toLocaleString(), icon: ShoppingCart, color: "text-blue-500" },
    { label: "Customers", value: data.customers.toLocaleString(), icon: Users, color: "text-violet-500" },
    { label: "Avg. Order", value: formatCurrency(data.avgOrderValue), icon: TrendingUp, color: "text-amber-500" },
  ]
            ...
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.topProducts.slice(0, 5)} layout="vertical">
                  <XAxis type="number" tickFormatter={(v) => `$${v}`} tick={{ fill: '#888', fontSize: 10 }} />
                  <YAxis 
                    type="category" 
                    dataKey="product" 
                    width={80}
              ...
              {data.bySource.slice(0, 6).map((source, i) => {
                ...
                return (
                  <div key={source.source} className="flex items-center gap-3">
                    <div className="w-20 text-xs text-muted-foreground truncate">{source.source}</div>
                    ...
                    <div className="w-20 text-right text-xs font-medium">{formatCurrency(source.revenue)}</div>
                  </div>
                )
              })}
            </div>
