          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="p-3 rounded-lg bg-muted/30 text-center">
                <p className="text-lg font-bold text-foreground">{data.visitors.toLocaleString()}</p>
                <p className="text-xs text-muted-foreground">Visitors</p>
              </div>
              <div className="p-3 rounded-lg bg-muted/30 text-center">
                <p className="text-lg font-bold text-foreground">{data.pageviews.toLocaleString()}</p>
                <p className="text-xs text-muted-foreground">Pageviews</p>
              </div>
              <div className="p-3 rounded-lg bg-muted/30 text-center">
                <p className="text-lg font-bold text-emerald-500">${data.revenue.toLocaleString()}</p>
            ...
            {data.planDistribution && data.planDistribution.length > 0 && (
              <div>
                <h4 className="text-xs font-medium text-muted-foreground mb-2 uppercase tracking-wider">Plan Distribution</h4>
                <div className="space-y-2">
                  {data.planDistribution.map((plan) => {
                    const total = data.planDistribution.reduce((s, p) => s + p.visitors, 0)
                    const percentage = total > 0 ? (plan.visitors / total) * 100 : 0
                    const color = plan.plan === 'pro' ? 'bg-amber-500' : 
                                  plan.plan === 'free' ? 'bg-emerald-500' : 'bg-blue-500'
                    
                    return (
                      <div key={plan.plan} className="flex items-center gap-2">
                        <span className="w-16 text-xs text-muted-foreground capitalize">{plan.plan}</span>
