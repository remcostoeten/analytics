...
import type { DashboardData, BreadcrumbItem, SignalEvent, KPIMetric, GeoDistribution } from "@/lib/types"
import {
  ChevronRight, BarChart3, Users, Settings2,
  CalendarDays, Route, Radio
} from "lucide-react"
...
export function DashboardContent({
  ...
  const { data: projects } = useSWR(
    ...
    fetcher,
    { fallbackData: [], refreshInterval: 60000 }
  )

  const { data: overview } = useSWR(
    buildQuery("overview-extended"),
    fetcher,
    { fallbackData: null, refreshInterval: 30000 }
  )
  ...
  const { data: webVitals } = useSWR(
    buildQuery("web-vitals"),
    fetcher,
    { fallbackData: null, refreshInterval: 60000 }
  )

  const { data: sessionStats } = useSWR(
    buildQuery("session-stats"),
    fetcher,
    { fallbackData: null, refreshInterval: 30000 }
  )

  const { data: utmCampaigns } = useSWR(
    buildQuery("utm-campaigns"),
    fetcher,
    { fallbackData: [], refreshInterval: 30000 }
  )

  const { data: engagement } = useSWR(
    buildQuery("engagement"),
    fetcher,
    { fallbackData: null, refreshInterval: 30000 }
  )

  const { data: heatmap } = useSWR(
    buildQuery("hourly-heatmap"),
    fetcher,
    { fallbackData: null, refreshInterval: 60000 }
  )

  const { data: browsers } = useSWR(
    buildQuery("browsers-detailed"),
    fetcher,
    { fallbackData: initialData.audience.browsers, refreshInterval: 60000 }
  )

  const { data: operatingSystems } = useSWR(
    buildQuery("os-detailed"),
    fetcher,
    { fallbackData: initialData.audience.os, refreshInterval: 60000 }
  )

  const { data: languages } = useSWR(
    buildQuery("languages"),
    fetcher,
    { fallbackData: initialData.audience.languages, refreshInterval: 60000 }
  )

  const { data: screenSizes } = useSWR(
    buildQuery("screen-sizes"),
    fetcher,
    { fallbackData: initialData.audience.screenResolutions, refreshInterval: 60000 }
  )

  const { data: connectionTypes } = useSWR(
    buildQuery("connection-types"),
    fetcher,
    { fallbackData: [], refreshInterval: 60000 }
  )
  ...
  const { data: entryExitPages } = useSWR(
    buildQuery("entry-exit-pages"),
    fetcher,
    { fallbackData: null, refreshInterval: 30000 }
  )

  const { data: liveNow } = useSWR(
    buildQuery("live-now"),
    fetcher,
    { fallbackData: null, refreshInterval: 5000 }
  )

  const { data: retention } = useSWR(
    buildQuery("retention"),
    fetcher,
    { fallbackData: null, refreshInterval: 60000 }
  )
  ...
  const paletteReferrers = useMemo(() => {
    ...
    return referrers.map((r: { domain: string; visits: number }) => ({ domain: r.domain, visits: r.visits }))
  }, [referrers])
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-2xl font-bold text-foreground">{selectedCountry.count.toLocaleString()}</p>
                <p className="text-xs text-muted-foreground">Events</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-foreground">{selectedCountry.visitors.toLocaleString()}</p>
            ...
            <div className="mt-4 pt-4 border-t border-border">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-foreground">{selectedCountry.percentage.toFixed(1)}%</span>
              </div>
            </div>
            <button
              onClick={() => setSelectedCountry(null)}
