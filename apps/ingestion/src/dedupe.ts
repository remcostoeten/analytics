...
export class DedupeCache {
	...
	private cleanup(): void {
		...
		if (removedCount > 0) {
			console.log(
				`[Dedupe] Cleaned up ${removedCount} expired entries. Cache size: ${this.cache.size}`,
			);
		}
	}
...
export class DedupeMetrics {
	...
	getMetrics() {
		const hitRate = this.totalRequests > 0 ? this.duplicatesBlocked / this.totalRequests : 0;
