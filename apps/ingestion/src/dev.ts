...
function request(path: string) {
	...
	fetch(baseUrl + path)
		.then(async function (res) {
			...
			console.log(
				`  ${statusColor}◄ ${res.status} ${res.statusText} ${colors.dim}(${time}ms)${colors.reset}\n`,
			);
...
function info() {
	console.log(`\n  ${colors.bold}System & Environment${colors.reset}`);
	...
	console.log(`  ${pad("NPM Link:", 15)} ${colors.blue}${npmUrl}${colors.reset}`);
	console.log(`  ${pad("Repo Link:", 15)} ${colors.blue}${repoUrl}${colors.reset}`);
	console.log(`  ${pad("Commit Hash:", 15)} ${colors.yellow}${gitHash}${colors.reset}`);
	console.log(`  ${pad("Commit Date:", 15)} ${colors.dim}${gitDate}${colors.reset}`);
	console.log(`  ${pad("Message:", 15)} ${colors.dim}${gitMessage}${colors.reset}`);
...
const ENDPOINTS = [
	{ method: "GET ", path: "/", desc: "Returns HTML dashboard overview" },
	{ method: "GET ", path: "/health", desc: "Basic healthcheck verifying the status" },
	{ method: "POST", path: "/ingest", desc: "Ingest generic platform events payload" },
	{ method: "GET ", path: "/metrics", desc: "Returns rate-limiter and cache hit metrics" },
	{ method: "GET ", path: "/admin/stats", desc: "Detailed DB sizes and retention limits" },
	{ method: "POST", path: "/admin/cleanup", desc: "Manually triggers cache/retention clean" },
	{ method: "GET ", path: "/events", desc: "SSE Live streaming dashboard updates" },
];
...
function prompt() {
	...
	for (let i = 0; i < cmds.length; i += 2) {
		...
		const col2 = cmds[i + 1]
			? `${colors.cyan}${cmds[i + 1].key}${colors.reset} ${colors.dim}·${colors.reset} ${cmds[i + 1].desc}`
			: "";
